...
IF OBJECT_ID ('dbo.LOAD_RELOC_POLICY_TYP_WDJOBAPPLICATION_EXTRACT') IS NOT NULL
DROP PROCEDURE [dbo].[LOAD_RELOC_POLICY_TYP_WDJOBAPPLICATION_EXTRACT]
GO


CREATE PROCEDURE [dbo].[LOAD_RELOC_POLICY_TYP_WDJOBAPPLICATION_EXTRACT]
AS
BEGIN
----------------------------------------------------------------------------------------------------------------------------------------------------
--  Name              : LOAD_RELOC_POLICY_TYP_WDJOBAPPLICATION_EXTRACT
--  Purpose           : 1. Synchronize between 
--                         RELOC_POLICY_TYP and WDJobApplicationExtract
...
--							records.
--	Sample Call       : EXEC  [PEOPLE_ODS].[dbo].[LOAD_RELOC_POLICY_TYP_WDJOBAPPLICATION_EXTRACT]					
--
...
BEGIN TRY
	...
	DECLARE  @TARGET_TBL_NAME		    VARCHAR(250) = 'RELOC_POLICY_TYP'
	...
	(	MergeAction VARCHAR(150)
		,RELOC_POLICY_TYP_NAME				VARCHAR(510)
	...
	BEGIN TRY
		...
		EXEC [ETL].[WRITE_ETL_EXECUTION_LOG] @TASK_NAME=@spname 
											,@SRC_FILE_DATE=@SRC_FILE_DATE_JOB_APP
											,@EventName='START'
											,@EVENT_SQL_STATEMENT='SELECT * INTO  #RELOC_POLICY_TYP FROM HRDWStaging.dbo.WDJobApplicationExtract WITH (NOLOCK)'
											,@Message='LOADING DATA INTO SESSION TABLE WITH ALL DATA TYPE CONVERSIONS IS STARTED'
											,@EVENT_TIME=@EVENTTIME
											,@RecordsAffected = 0
											,@RecordsInserted = 0
											,@RecordsUpdated  = 0
		IF OBJECT_ID ('tempdb..#RELOC_POLICY_TYP') IS NOT NULL
		DROP TABLE #RELOC_POLICY_TYP;	
		 
		SELECT DISTINCT dbo.TrimBlankNull(RELOCATIONPOLICY) AS RELOC_POLICY_TYP_NAME
		INTO  #RELOC_POLICY_TYP
		FROM [HRDWStaging].[dbo].[WDJOBAPPLICATIONEXTRACT] WITH (NOLOCK) 
		WHERE dbo.TrimBlankNull(RELOCATIONPOLICY) IS NOT NULL 
    ...
    BEGIN TRY
        ...
        EXEC [ETL].[WRITE_ETL_EXECUTION_LOG] @TASK_NAME=@spname 
                                            ,@SRC_FILE_DATE=@SRC_FILE_DATE_JOB_APP
                                            ,@EventName='START'
                                            ,@EVENT_SQL_STATEMENT = '  MERGE  PEOPLE_ODS.DBO.RELOC_POLICY_TYP AS DEST 
																	  USING #RELOC_POLICY_TYP AS SRC 
																	  ON UPPER(DEST.RELOC_POLICY_TYP_NAME)	= UPPER(SRC.RELOC_POLICY_TYP_NAME) '
                                            ...
                                            ,@RecordsUpdated  = 0
		BEGIN TRAN
			MERGE  PEOPLE_ODS.DBO.RELOC_POLICY_TYP AS DEST 
	        USING #RELOC_POLICY_TYP AS SRC 
	        ON UPPER(DEST.RELOC_POLICY_TYP_NAME)	= UPPER(SRC.RELOC_POLICY_TYP_NAME)		
            WHEN NOT MATCHED BY TARGET THEN
            INSERT 
            (
				 RELOC_POLICY_TYP_NAME        
                ,ETL_CREATE_DATETM
