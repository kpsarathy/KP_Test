...
IF OBJECT_ID ('dbo.LOAD_JOB_OFFR_DECLN_REASN_TYP_WDJOBAPPLICATION_EXTRACT') IS NOT NULL
DROP PROCEDURE [dbo].[LOAD_JOB_OFFR_DECLN_REASN_TYP_WDJOBAPPLICATION_EXTRACT]
GO


CREATE PROCEDURE [dbo].[LOAD_JOB_OFFR_DECLN_REASN_TYP_WDJOBAPPLICATION_EXTRACT]
AS
BEGIN
----------------------------------------------------------------------------------------------------------------------------------------------------
--  Name              : LOAD_JOB_OFFR_DECLN_REASN_TYP_WDJOBAPPLICATION_EXTRACT
...
--							records.
--	Sample Call       : EXEC  [PEOPLE_ODS].[dbo].[LOAD_JOB_OFFR_DECLN_REASN_TYP_WDJOBAPPLICATION_EXTRACT]					
--
...
BEGIN TRY
	...
	DECLARE  @TARGET_TBL_NAME		    VARCHAR(250) = 'JOB_OFFR_DECLN_REASN_TYP'
	...
	(	MergeAction VARCHAR(150)
		,JOB_OFFR_DECLN_REASN_TYP_NAME				VARCHAR(510)
	...
	BEGIN TRY
		...
		EXEC [ETL].[WRITE_ETL_EXECUTION_LOG] @TASK_NAME=@spname 
											,@SRC_FILE_DATE=@SRC_FILE_DATE_JOB_APP
											,@EventName='START'
											,@EVENT_SQL_STATEMENT='SELECT * INTO  #JOB_OFFR_DECLN_REASN_TYP FROM HRDWStaging.dbo.WDJobApplicationExtract WITH (NOLOCK)'
											,@Message='LOADING DATA INTO SESSION TABLE WITH ALL DATA TYPE CONVERSIONS IS STARTED'
											,@EVENT_TIME=@EVENTTIME
											,@RecordsAffected = 0
											,@RecordsInserted = 0
											,@RecordsUpdated  = 0
		IF OBJECT_ID ('tempdb..#JOB_OFFR_DECLN_REASN_TYP') IS NOT NULL
		DROP TABLE #JOB_OFFR_DECLN_REASN_TYP;	

		SELECT DISTINCT dbo.TrimBlankNull(OFFERDECLINEREASON) AS  JOB_OFFR_DECLN_REASN_TYP_NAME
		INTO  #JOB_OFFR_DECLN_REASN_TYP
		FROM [HRDWStaging].[dbo].[WDJOBAPPLICATIONEXTRACT] WITH (NOLOCK) 
		WHERE dbo.TrimBlankNull(OFFERDECLINEREASON) IS NOT NULL 
    ...
    BEGIN TRY
        ...
        EXEC [ETL].[WRITE_ETL_EXECUTION_LOG] @TASK_NAME=@spname 
                                            ,@SRC_FILE_DATE=@SRC_FILE_DATE_JOB_APP
                                            ,@EventName='START'
                                            ,@EVENT_SQL_STATEMENT = '  MERGE  PEOPLE_ODS.DBO.JOB_OFFR_DECLN_REASN_TYP AS DEST 
																	  USING #JOB_OFFR_DECLN_REASN_TYP AS SRC 
																	  ON UPPER(DEST.JOB_OFFR_DECLN_REASN_TYP_NAME)	= UPPER(SRC.JOB_OFFR_DECLN_REASN_TYP_NAME) '
                                            ...
                                            ,@RecordsUpdated  = 0
		BEGIN TRAN
			MERGE  PEOPLE_ODS.DBO.JOB_OFFR_DECLN_REASN_TYP AS DEST 
	        USING #JOB_OFFR_DECLN_REASN_TYP AS SRC 
	        ON UPPER(DEST.JOB_OFFR_DECLN_REASN_TYP_NAME)	= UPPER(SRC.JOB_OFFR_DECLN_REASN_TYP_NAME)		
            WHEN NOT MATCHED BY TARGET THEN
            INSERT 
            (
				 JOB_OFFR_DECLN_REASN_TYP_NAME        
                ,ETL_CREATE_DATETM
            ...
            VALUES 
            (
				 SRC.JOB_OFFR_DECLN_REASN_TYP_NAME        
                ,@ETL_CREATE_DATETM
                ,@ETL_CREATE_EMP_LOGIN_NAME
                ,@ETL_UPDATE_DATETM
                ,@ETL_UPDATE_EMP_LOGIN_NAME
